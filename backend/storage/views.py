...
class FileViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['patch'], url_path='update_filename')
    def update_filename(self, request, pk=None):
        ...
        try:
            ...
            if new_filename is None:
                return Response(
                    {"error": "Поле 'filename' обязательно"},
                    status=status.HTTP_400_BAD_REQUEST
                )

            file.original_name = new_filename
            file.save()
