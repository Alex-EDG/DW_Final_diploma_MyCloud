...
export const updateFileComment = createAsyncThunk(
  ...
  async ({ fileId, comment }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data || 'Ошибка обновления комментария');
    };
  }
);
...
export const updateFileName = createAsyncThunk(
  ...
  async ({ fileId, filename }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data || 'Ошибка обновления имени файла');
    };
  }
);
