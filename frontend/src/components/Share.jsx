...
const ShareDownloadPage = () => {
  ...
  useEffect(() => {
    const fetchFileInfo = async () => {
      try {
        ...
        setError(error.response?.data?.error || 'Не удалось загрузить информацию о файле');
      } finally {
        setLoading(false);
      };
    };
  ...
  const handleDownload = async () => {
    try {
      ...
      message.error(error.response?.data?.error || 'Ошибка при скачивании файла');
    }
  };
    ...
    <div className="download-container">
        <Card title="Скачать файл">
            <div className="file-info">
                ...
                <p><strong>Комментарий к файлу: </strong>{ fileInfo.comment || 'нет комментария' }
                </p>
            </div>
